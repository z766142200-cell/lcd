...
typedef int (*SCCB_Wtrename)( uint16_t , uint8_t );
typedef int (*SCCB_Rdrename)(uint8_t );
SCCB_Wtrename SCCB_WR_Reg = SCCB_WriteByte;
SCCB_Rdrename SCCB_RD_Reg = SCCB_ReadByte;
...
{
	OV7725_Init();
	OV7725_Light_Mode(4);		//设置为室内模式
	OV7725_Color_Saturation(4);	//饱和度设置为+4
	OV7725_Brightness(4);		//亮度设置为+4
	OV7725_Contrast(4);			//对比度设置为+4
	OV7725_Special_Effects(0);	//设置为普通模式
	// OV7725_Window_Set(320,240,0);//设置输出窗口为320*240
